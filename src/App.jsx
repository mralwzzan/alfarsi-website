...
import { Menu, X, Phone, Mail, MapPin, Clock, ArrowRight, Check } from 'lucide-react';

export default function AlFarsiLawOffice() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [showBookingForm, setShowBookingForm] = useState(false);
  const [selectedService, setSelectedService] = useState('احوال شخصية');
    ...
    <div className="w-full bg-white">
      ...
      {showBookingForm && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl p-8 w-full max-w-2xl border border-slate-200 shadow-2xl my-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold text-slate-800">📅 احجز استشارتك</h2>
              <button
            ...
            <form onSubmit={handleBookingSubmit} className="space-y-4">
              <div className="bg-blue-50 p-4 rounded-xl mb-6 border border-blue-200">
                <p className="text-slate-700 font-semibold mb-3">نوع الاستشارة:</p>
                <select
                  value={bookingData.consultationType}
                  onChange={(e) => setBookingData({...bookingData, consultationType: e.target.value})}
                  className="w-full bg-white border border-blue-300 text-slate-800 px-4 py-3 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 font-bold text-lg"
                >
                  <option value="احوال شخصية">👨‍⚖️ احوال شخصية - 300 ر.س</option>
                  <option value="تجارية">💼 تجارية - 750 ر.س</option>
                  <option value="عامة">📋 عامة - 500 ر.س</option>
                </select>
                <p className="text-slate-600 text-sm mt-3">⏱️ مدة الاستشارة: ساعة كاملة</p>
              </div>
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-2">تفاصيل إضافية (اختياري)</label>
                <textarea
                  value={bookingData.description}
                  onChange={(e) => setBookingData({...bookingData, description: e.target.value})}
                  className="w-full bg-slate-50 border border-slate-300 text-slate-800 px-4 py-3 rounded-xl focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 h-24 resize-none"
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="button"
                  onClick={() => setShowBookingForm(false)}
                  className="flex-1 bg-slate-300 hover:bg-slate-400 text-slate-800 font-bold py-3 px-4 rounded-xl transition"
                >
                  ✕ إلغاء
                </button>
              </div>
      ...
      <nav className="fixed w-full top-0 bg-white shadow-sm z-50">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-3">
            <button onClick={() => openBookingForm('احوال شخصية')} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-semibold transition">
              احجز الآن
            </button>
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white border-t border-slate-200 py-4 px-4 space-y-3">
            ...
            <button onClick={() => {setIsMenuOpen(false); openBookingForm('احوال شخصية');}} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-lg font-semibold text-center transition">احجز الآن</button>
          </div>
        )}
      </nav>
